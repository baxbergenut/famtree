...
const CENTER_ROOT_BUTTON_OFFSET = 24;
...
export function WorkspaceCanvas() {
  ...
  function centerOnRoot(targetGraph: TreeGraph | null) {
    if (!targetGraph) {
      return;
    }

    const root = targetGraph.persons.find(
    ...
    const centerX =
      (window.innerWidth - SIDEBAR_WIDTH - SIDEBAR_GAP * 2) / 2;
      ...
      <section className="flex min-h-screen items-center justify-center bg-[var(--background)] px-6">
        <div className="w-full max-w-2xl rounded-[32px] border border-[var(--line-soft)] bg-white/80 p-8 shadow-[0_20px_80px_rgba(31,27,24,0.08)]">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-7 text-[var(--ink-soft)]">
            The workspace needs an active session before it can load your root
    ...
    <div className="fixed inset-0 overflow-hidden bg-[var(--background)]">
      <div
        ...
        style={{
          ...
          backgroundPosition: `${camera.x % 48}px ${camera.y % 48}px, ${camera.x % 48}px ${camera.y % 48}px, center`,
        }}
      >
        <svg className="absolute inset-0 h-full w-full pointer-events-none" aria-hidden="true">
          {relationships.map((relationship) => {
            ...
            const parentX = parent.x + camera.x;
            const parentY = parent.y + camera.y + NODE_HALF_HEIGHT;
            const childX = child.x + camera.x;
            const childY = child.y + camera.y - NODE_HALF_HEIGHT;
        ...
        {persons.map((person) => (
          <div
            ...
            style={{
              left: person.x + camera.x,
              top: person.y + camera.y,
            }}
          >
            <PersonNodePreview
      ...
      <aside className="fixed bottom-6 right-6 top-[108px] z-30 w-[360px] overflow-auto rounded-[32px] border border-[var(--line-soft)] bg-white/84 p-6 shadow-[0_24px_80px_rgba(31,27,24,0.12)] backdrop-blur">
        ...
        <p className="mt-3 text-sm leading-7 text-[var(--ink-soft)]">
          Click a node and use its action buttons. You can also drag the canvas
          to navigate and drag nodes to reposition them.
        </p>
        ...
        {draft ? (
            ...
            <div className="grid gap-4">
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={updateDraft}
                  type="text"
                  placeholder={draft.relation === "parent" ? "mom, dad, guardian..." : "son, daughter, child..."}
                  className="w-full rounded-2xl border border-[var(--line-soft)] bg-white px-4 py-3 text-sm text-[var(--ink-strong)] outline-none transition focus:border-[var(--accent-strong)]"
                />
              </label>
              <label className="block">
          ...
          <div className="mt-6 rounded-[24px] border border-dashed border-[var(--line-strong)] bg-[var(--surface-muted)]/60 p-5 text-sm leading-7 text-[var(--ink-soft)]">
            Start from the highlighted root node in the middle of the canvas,
            then add parents above or children below. Drag anywhere on the
            background to move through the canvas.
          </div>
        )}
      </aside>
      ...
      <button
        ...
        style={{ bottom: CENTER_ROOT_BUTTON_OFFSET, left: CENTER_ROOT_BUTTON_OFFSET }}
      >
        Center root
      </button>
    </div>
